        ...
        <aside className="flex flex-col h-full w-64 border-r border-border/50 bg-card/30 backdrop-blur-sm">
            ...
            {savedPrompts.length > 0 && (
                <>
                    <Separator className="bg-border/50" />
                    <div className="p-3">
                        <div className="text-xs font-semibold text-muted-foreground mb-2 flex items-center gap-2 px-1">
                        ...
                        <ScrollArea className="max-h-40">
                            <div className="space-y-1">
                                {savedPrompts.map((prompt, index) => (
                                    <div
                                        key={index}
                                        className="group flex items-center gap-2 rounded-lg px-2 py-1.5 cursor-pointer hover:bg-accent/50 text-sm transition-all"
                                        onClick={() => createMutation.mutate(prompt)}
                                    >
                                        <div className="flex-1 truncate text-xs text-foreground/70 group-hover:text-foreground">
                                            {prompt}
                                        </div>
                                        <Button
                                            size="icon"
                                            variant="ghost"
                                            className="w-5 h-5 opacity-0 group-hover:opacity-100 hover:text-destructive shrink-0"
                                            onClick={(e) => {
                                                e.stopPropagation();
